...
class UserInvitationadmin extends CI_Controller {

	public function __construct()
	{ 
		parent::__construct();
		$this->load->model('UserInvitationadmin_model');
		...
		if ($post_user) 
			{
				if($post_user['UserId']>0)
				{
					...
					$post_user['Code']= $res;
					$result = $this->UserInvitationadmin_model->edit_user($post_user);
					if($result)
					{
								...
								foreach($query->result() as $row){ 
									...
									//$body = str_replace("{ code }",$body,$post_user['Code']);					
				...
				else
				{
						...
						$result = $this->UserInvitationadmin_model->add_user($post_user); 
	...
	{
			$data=$this->UserInvitationadmin_model->getlist_user();
	...
	{
			$data=$this->UserInvitationadmin_model->getlist_inviteduser();
			echo json_encode($data);
	}
	...
	public function delete() {
		...
		if ($post_revoke)
		 {
			if($post_revoke['id'] > 0){
				$result = $this->UserInvitationadmin_model->delete_Invitation($post_revoke);
				if($result) {
	...
	public function ReInvite() {
		...
		if(!empty($post_user)) {
			...
			$post_user['Code']= $res;
			$result = $this->UserInvitationadmin_model->ReInvite_Invitation($post_user);			
			if($result) {
	...
	public function deleteUser() {
		...
		if ($post_user)
		 {
			if($post_user['id'] > 0){
				$result = $this->UserInvitationadmin_model->delete_user($post_user);
				if($result) {	
					echo json_encode("Delete successfully");
					}
		 	}
	...
	{	
		...
		{
			$data=[];
			$data=$this->UserInvitationadmin_model->get_userdata($User_Id);
			echo json_encode($data);
		}
	}
	...
	{
		//$data="";
		$data['company']=$this->UserInvitationadmin_model->getlist_company();
		$data['department']=$this->UserInvitationadmin_model->getlist_department();
		$data['role']=$this->UserInvitationadmin_model->getlist_userrole();
	//	$data['country']=$this->UserInvitationadmin_model->getlist_country();
	//	$data['state']=$this->UserInvitationadmin_model->getlist_state();
		echo json_encode($data);
	}
