...
class Userinvite extends CI_Controller {
		...
		if ($post_user) 
			{
				...
				{
					$result = $this->Userinvite_model->edit_user($post_user);
					if($result)
					{
				...
				else
				{
						...
						$result = $this->Userinvite_model->add_user($post_user); 
	...
	{
			$data=$this->Userinvite_model->getlist_user();
	...
	{
			$data=$this->Userinvite_model->getlist_inviteduser();
			echo json_encode($data);
	}
	...
	public function delete() {
		...
		if ($post_revoke)
		 {
			if($post_revoke['id'] > 0){
				$result = $this->Userinvite_model->delete_Invitation($post_revoke);
				if($result) {
	...
	public function ReInvite() {
		...
		if(!empty($post_user)) {
			...
			$post_user['Code']= $res;
			$result = $this->Userinvite_model->ReInvite_Invitation($post_user);			
			if($result) {
	...
	public function getAllDefaultData()
	{
		...
		$data['department']=$this->Userinvite_model->getlist_department();
		$data['role']=$this->Userinvite_model->getlist_userrole();
		$data['industry']=$this->Userinvite_model->getlist_industry();
		$data['parentcomp']=$this->Userinvite_model->getlist_allcompany();
